...
const Text = styled.span`
	color: ${(props) => (props.price ? '#343a40' : '#848c94')};
	font-size: ${(props) => (props.price ? '24px' : '14px')};
...
const Wrap = styled.div`
	padding: ${(props) => {
		...
		} else if (props.hotelName) {
			return '24px 16px';
		} else {
			return '0';
		}
	}};
	border-top: ${(props) => (props.hotelName ? '1px solid #dee2e6' : 'none')};
	display: ${(props) => (props.hotelName || props.mail ? 'flex' : 'block')};
	justify-content: ${(props) =>
		props.hotelName || props.mail ? 'space-between' : 'center'};
	align-items: center;
`;
...
const Button = styled.button.attrs({
	...
	height: ${(props) => (props.booking ? '48px' : '40px')};
	border: ${(props) =>
		props.booking ? '1px solid var(--color-theme)' : '1px solid #ced4da'};
	color: ${(props) => (props.booking ? '#fff' : '#495056')};
	background-color: ${(props) =>
		props.booking ? 'var(--color-theme)' : 'white'};
	font-size: ${(props) => (props.booking ? '16px' : '14px')};
	line-height: ${(props) => (props.booking ? '24px' : '21px')};
...
const ShareIcon = styled(ShareFill)`
	width: 20px;
	height: 20px;
`;

const WishHeart = styled(Heart)`
	width: 20px;
	height: 20px;
	margin-right: 4px;
	color: #ced4da;
